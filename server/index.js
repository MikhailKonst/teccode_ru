...
import { isMailConfigured, sendContactEmail, verifyMail } from './lib/mailer.js';
...
const VALID_TOPICS = new Set(['order', 'support']);
...
app.post('/api/contact', rateLimitContact, asyncHandler(async (req, res) => {
	...
	const rawTopic   = String(req.body?.topic   ?? 'order').trim();
	const topic      = VALID_TOPICS.has(rawTopic) ? rawTopic : 'order';
	...
	if (isMailConfigured()) {
		try {
			await sendContactEmail({ name: rawName, email: rawEmail, message: rawMessage, topic });
			await prisma.contactSubmission.update({
				where: { id: submission.id },
				data: { emailSent: true },
			});
		} catch (err) {
			// Заявка уже сохранена в БД и видна в админке — лид не теряется,
			// даже если письмо не ушло (частая причина — провайдер/IP блокирует SMTP).
			console.error('[server] email send failed:', err?.code ?? '', err?.message ?? err);
		}
	}
...
app.post('/api/auth/login', rateLimitLogin, asyncHandler(async (req, res) => {
	...
	const loginOk = timingSafeEqual(login, creds.login);
...
const CONTACT_FILTERS = new Set(['all', 'read', 'unread']);

app.get('/api/admin/contacts', requireAuth(), asyncHandler(async (req, res) => {
	const page   = Math.max(1, Number(req.query.page ?? 1));
	const filter = CONTACT_FILTERS.has(String(req.query.filter ?? 'all')) ? String(req.query.filter ?? 'all') : 'all';
	const limit  = 50;
	const skip   = (page - 1) * limit;

	const where = filter === 'read'   ? { isRead: true }
	...
	res.json({ items, total, unreadCount, page, pages: Math.ceil(total / limit) });
}));

app.patch('/api/admin/contacts/:id/read', requireAuth(), asyncHandler(async (req, res) => {
	const { id } = req.params;
...
app.delete('/api/admin/contacts/:id', requireAuth(), asyncHandler(async (req, res) => {
	const { id } = req.params;
	const existing = await prisma.contactSubmission.findUnique({ where: { id }, select: { id: true } });
	if (!existing) return res.status(404).json({ error: 'not_found' });
...
app.delete('/api/admin/contacts', requireAuth(), asyncHandler(async (req, res) => {
	const filter = CONTACT_FILTERS.has(String(req.query.filter ?? '')) ? String(req.query.filter) : null;
	if (!filter || filter === 'all') {
		await prisma.contactSubmission.deleteMany({});
	} else {
		await prisma.contactSubmission.deleteMany({ where: { isRead: filter === 'read' } });
	}
	res.json({ ok: true });
}));
...
const server = app.listen(port, async () => {
	...
	if (!isMailConfigured()) {
		console.log('[mail] ✗ SMTP не настроен — заявки сохраняются в БД, письма не отправляются. Заполни SMTP_HOST/USER/PASS в .env');
		return;
	}

	const orders = process.env.MAIL_TO_ORDERS ?? 'info@teccode.ru';
	const support = process.env.MAIL_TO_SUPPORT ?? 'support@teccode.ru';

	// Реальная проверка соединения/аутентификации, чтобы сразу видеть проблему.
	const check = await verifyMail();
	if (check.ok) {
		console.log(`[mail] ✓ SMTP работает → заказы: ${orders}, поддержка: ${support}`);
	} else {
		console.warn(
			`[mail] ⚠ SMTP настроен, но проверка не прошла (${check.reason}). ` +
				'Письма отправляться НЕ будут, заявки сохраняются в БД. ' +
				'Частые причины: провайдер/хостинг/VPN блокирует исходящий SMTP, ' +
				'неверный пароль приложения или порт. Проверь SMTP_* в .env.',
		);
	}
});
