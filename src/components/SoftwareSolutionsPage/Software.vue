<template>
	<section class="products">
		<div class="products__container">
			...
			<div v-else ref="gridEl" class="products__grid" :class="{ 'products__grid--revealed': revealed }">
				<RouterLink
...
const { el: gridEl, revealed } = useScrollReveal(0.05);
...
.products {
	...
	&__grid {
		> .products__card {
			...
			transition: opacity 0.55s ease, transform 0.55s ease, border-color 0.15s ease-out, box-shadow 0.15s ease-out;
		}

		@for $i from 1 through 20 {
			&--revealed > .products__card:nth-child(#{$i}) {
				opacity: 1;
				transform: none;
