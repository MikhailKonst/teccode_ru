<template>
	<form class="form" @submit.prevent="onSubmit">
		...
		<div class="form__field form__field--full">
			<label for="topic" class="form__label">Тема обращения</label>
			<select
				id="topic"
				v-model="topic"
				class="form__select"
				:disabled="loading"
			>
				<option value="order">Заказать продукт / запросить цену</option>
...
import { contactApi, type ContactTopic } from '@/api/contact';
...
const email = ref('');
const message = ref('');
const topic = ref<ContactTopic>('order');
...
.form {
	...
	&__textarea {
		...
		/* Убираем фон браузерного автофилла — бесконечная задержка перехода */
		&:-webkit-autofill,
