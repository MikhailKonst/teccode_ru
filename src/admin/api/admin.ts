...
export const adminApi = {
	...
	getContacts: (token: string, page = 1, filter: 'all' | 'read' | 'unread' = 'all') =>
		http.get<{
			...
			page: number;
			pages: number;
		}>(`/api/admin/contacts?page=${page}&filter=${filter}&ts=${Date.now()}`, token),

	markContactRead: (id: string, token: string) =>
		http.patch<{ item: ContactSubmission }>(`/api/admin/contacts/${encodeURIComponent(id)}/read`, undefined, token),

	deleteContact: (id: string, token: string) =>
		http.delete<{ ok: true }>(`/api/admin/contacts/${encodeURIComponent(id)}`, token),

	deleteContactsByFilter: (filter: 'read' | 'unread' | 'all', token: string) =>
		http.delete<{ ok: true }>(`/api/admin/contacts?filter=${filter}`, token),
};
