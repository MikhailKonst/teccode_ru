	...
	<section class="admin">
		<div class="admin__container">
			...
			<nav class="admin__nav">
				...
				<RouterLink :to="{ name: ROUTES.adminContacts.name }" class="admin__nav-item">
					Журнал заявок
				</RouterLink>
			</nav>
					...
					>
						...
						<label class="admin__label">
							...
							<span class="admin__hint"
								>PNG/JPG/WebP/GIF, до 3 МБ. Изображение вставится в редактор.</span
							>
...
import { toErrorMessage, isImagePreview, fileToDataUrl } from '@/admin/utils';
...
const docEditor = ref<InstanceType<typeof RichTextEditor> | null>(null);
...
async function onDocImageFile(e: Event) {
	...
	try {
		...
		docEditor.value?.insertImage(url);
		docImageFileName.value = '';
